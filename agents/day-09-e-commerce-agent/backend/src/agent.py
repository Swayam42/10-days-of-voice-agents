...
def load_catalog():
    """Load product catalog from catalog.json file."""
    try:
...
async def show_catalog(
    ...
    # Summarize top 8
    lines = [f"Here are the top {min(8, len(prods))} items I found at Sahoo Babu Shop:"]
...
async def show_catalog(
    ...
    if not prods:
        return "Sorry — I couldn't find any items that match. Would you like to try another search?"
    # Summarize top 4
    lines = [f"Here are the top {min(4, len(prods))} items I found at Sahoo Babu Shop:"]
...
class GameMasterAgent(Agent):
    def __init__(self):
        ...
        instructions = """
        You are Sahoo Babu, a smart and friendly personal shopping assistant helping customers discover amazing products.
        
        Your store offers: Electronics (headphones, speakers, smartwatches, power banks), Clothing (t-shirts, hoodies, jackets), 
        Home & Lifestyle items (water bottles, mugs, lamps, yoga mats), and Accessories (backpacks, wallets, sunglasses).
        
        Your personality:
        - Warm, conversational, and genuinely helpful
        - Keep responses natural and concise for voice clarity
        - Be enthusiastic but not pushy - make shopping feel effortless
        - Remember what's in their cart and reference it naturally
        
        Interaction guidelines:
        - Use tools: show_catalog, add_to_cart, show_cart, clear_cart, place_order, last_order
        - When showing products, mention product ID and price naturally (e.g., 'audio-001 at 2499 rupees')
        - Keep sentences short and voice-friendly - people are speaking, not typing
        - If they're browsing, ask what catches their interest
        - Before placing orders, confirm the items in a friendly way
        - Track their cart and remind them if they have items waiting
        
        Make every interaction smooth, helpful, and enjoyable!
        """
        super().__init__(
...
async def entrypoint(ctx: JobContext):
    ...
    logger.info("STARTING VOICE E-COMMERCE AGENT (Sahoo Babu Shop) — Sahoo Babu")
